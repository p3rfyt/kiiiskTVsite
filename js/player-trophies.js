...
(() => {
  ...
  if (!host || !playerTrophies.length) return;
  ...
  const cardMarkup = ({ trophy, team }) => {
    ...
    const details = [
      trophy.series ? `<span class="profile-trophy__series">${trophy.series}</span>` : "",
      trophy.date ? `<span>${trophy.date}</span>` : "",
      team ? `<span>Команда: ${team}</span>` : "",
      '<span class="profile-trophy__place">1 место</span>',
      trophy.href
    ...
    const inner = `
      <img class="profile-trophy__art" src="${trophy.image}" alt="${trophy.name}" loading="lazy">
  ...
  host.innerHTML = `
    <div class="profile-trophy-ribbon" aria-label="Победы на турнирах">
      ...
      <div class="profile-trophy-ribbon__track">${cards}</div>
    </div>
  `;
})();
